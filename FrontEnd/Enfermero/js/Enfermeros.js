...
.then(data =>{
    ...
    for(i=0;i<data.length;i++){
        text2+= `
                  ...
                  <td><button href="#" class="btn btn btn-success" onclick="adfads('${data[i].fechac})">Aceptar</button></td>
                  </tr>
                `
    }
    text2+=`</tbody>
  ...
  .then(data =>{
      ...
      for(i=0;i<data.length;i++){
          text+= `
                  ...
                  <td>${data[i].motivoc}</td>
                  <td><button href="#" class="btn btn btn-danger" onclick="eliminar('${data[i].fechac}','${data[i].horac}')">Declinar</button></td>
                  <td><button href="#" class="btn btn btn-success" onclick="eliminaddr('${data[i].fechac})">Aceptar</button></td>
                  </tr>
                  `
